...
export async function setupSidebar(canvas) { // Rendre async pour fetch
    ...
    if (!assetSearchInput || !availableAssetsList || !assetListPlaceholder) {
         console.error("Sidebar: Éléments de recherche, liste ou placeholder d'assets manquants.");
    ...
    canvas.on('mouse:down', (options) => {
        ...
        if (currentPlacementData && options.target) {
             console.log("Sidebar: Clic sur un objet existant, annulation du mode placement Code Géo.");
             cancelPlacementMode(canvas);
        }
        if (currentAssetPlacementData && options.target) {
            console.log("Sidebar: Clic sur un objet existant, annulation du mode placement Asset.");
    ...
    if (availableGeocodesList) {
        availableGeocodesList.addEventListener('dragstart', (e) => {
            ...
            if (targetItem) {
                ...
                console.log("Sidebar: Drag start Code Géo pour", targetItem.dataset.code);
            } else {
                e.preventDefault();
            }
        });
    ...
    if (assetSearchInput && availableAssetsList && assetListPlaceholder) {
        ...
        availableAssetsList.addEventListener('dragstart', (e) => {
             ...
             if (targetItem) {
                 ...
                 console.log("Sidebar: Drag start Asset pour", targetItem.dataset.name);
             } else {
                 e.preventDefault();
             }
         });
...
function renderAssetList(assets, listElement, placeholderElement) {
    ...
    if (!assets || assets.length === 0) {
        ...
        // Ne pas l'ajouter à la liste si elle est vide, laisser le ul vide
    ...
    assets.forEach(asset => {
        ...
        // Ajouter 'd-flex' pour aligner image et texte
        ...
        // Icône par défaut si miniature absente ou invalide
        ...
        img.alt = asset.name;
        img.style.width = '32px'; img.style.height = '32px'; img.style.objectFit = 'contain'; img.style.border = '1px solid #eee'; img.style.flexShrink = '0'; // Empêcher l'image de rétrécir

        const span = document.createElement('span');
        span.textContent = asset.name;
        span.style.overflow = 'hidden'; // Gérer les noms longs
