...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadBackgroundImage() {
        return new Promise((resolve, reject) => {
            if (!mapImageEl) {
                if (planType === 'image') reject(new Error("Élément #map-image non trouvé."));
                else resolve();
                return;
            }
    ...
    function loadSvgPlan(url) {
        return new Promise((resolve, reject) => {
            fabric.loadSVGFromURL(url, (objects, options) => {
                ...
                console.log("SVG group dimensions:", svgData.width, svgData.height, "Scale:", svgData.scaleX); // LOG
                fabricCanvas.setBackgroundColor('#ffffff');
                const canvasWidth = fabricCanvas.getWidth(),
                    canvasHeight = fabricCanvas.getHeight();
                console.log("Canvas dimensions after SVG load:", canvasWidth, canvasHeight); // LOG
                if (svgData.width && svgData.height && canvasWidth && canvasHeight) {
                    const scaleFactor = Math.min((canvasWidth * 0.9) / svgData.width, (canvasHeight * 0.9) / svgData.height, 1);
                    svgData.scale(scaleFactor);
                }
                fabricCanvas.add(svgData);
                svgData.center().setCoords();
                makeSvgGroupEditable(svgData);
                fabricCanvas.renderAll();
                resolve();
            }, null, {
                crossOrigin: 'anonymous'
    ...
    function makeSvgGroupEditable(obj) {
        ...
        if (obj.isType('group')) obj.getObjects().forEach(makeSvgGroupEditable);
    }

    function createInitialGeoTags() {
        console.log("Entering createInitialGeoTags. Background/SVG group:", fabricCanvas.backgroundImage || fabricCanvas.getObjects().find(o => o.type === 'group')); // LOG
        fabricObjects = {};
            ...
            if (code.placements) {
                code.placements.forEach(placement => {
                    if (placement.plan_id == currentPlanId && placement.pos_x !== null) {
                        console.log("Processing code for tag:", code.code_geo, "Placement:", placement); // LOG
                        const {
                            placements,
                            ...codeData
                        } = code;
    ...
    function createFabricTag(code) {
        const bg = fabricCanvas.backgroundImage || fabricCanvas.getObjects().find(o => o.isSvgBackground || o.type === 'group');
        console.log("createFabricTag - Found background/SVG:", bg); // LOG
        if (!bg || code.pos_x === null) {
            console.warn("createFabricTag - No background or pos_x null for", code.code_geo); // LOG
            return null;
        }
        const {
            left,
        ...
        console.log(`Calculated pixels for ${code.code_geo}: left=${left}, top=${top}`); // LOG
        ...
        if (code.anchor_x !== null) addArrowToTag(group, code.anchor_x, code.anchor_y);
    ...
    async function saveModifiedSvgPlan() {
        ...
        tempCanvas.add(...exportableObjects.map(obj => fabric.util.object.clone(obj)));
        ...
        try {
            ...
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            const result = await response.json();
            if (result.status === 'success') alert('Modifications SVG sauvegardées !');
            else throw new Error(result.message || 'Erreur serveur.');
        } catch (error) {
    ...
    function convertPercentToPixels(percentX, percentY) {
        ...
        return {
            left: (percentX / 100) * bgWidth,
            top: (percentY / 100) * bgHeight
        };
    }
    ...
    function convertPixelsToPercent(pixelX, pixelY) {
        ...
        return {
            posX: Math.max(0, Math.min(100, (relativeX / bgWidth) * 100)),
            posY: Math.max(0, Math.min(100, (relativeY / bgHeight) * 100))
        };
    }

    function addArrowToTag(tagGroup, anchorXPercent, anchorYPercent) {
        const bg = fabricCanvas.backgroundImage || fabricCanvas.getObjects().find(o => o.isSvgBackground || o.type === 'group');
        if (!bg || anchorXPercent === null) {
        ...
        const bgOrigin = bg.getPointByOrigin('left', 'top');
        const anchorGlobal = {
            x: bgOrigin.x + anchorPixels.left,
            y: bgOrigin.y + anchorPixels.top
        };
        const currentZoom = fabricCanvas.getZoom();
        if (tagGroup.arrowLine) {
            ...
            tagGroup.arrowLine = new fabric.Line([tagCenter.x, tagCenter.y, anchorGlobal.x, anchorGlobal.y], {
                stroke: '#34495e',
                strokeWidth: 2 / currentZoom,
                selectable: false,
                evented: false,
    ...
    function showToolbar(target) {
        ...
        tagToolbar.style.top = `${bound.top - tagToolbar.offsetHeight - 5}px`;
    ...
    async function deleteSelectedTag() {
        ...
        const allInstances = fabricCanvas.getObjects().filter(o => o.customData?.geoCodeId === geoCodeId);
    ...
    function updateHighlightEffect(fabricObj) {
        if (fabricObj.isGridLine) return;
